$(document).ready(function(){
    ...
    $(".submit").click(function() {
        // 13fedccf58a8c7c6a64dcf50cb406fa5
        const input = $("#input").val();
        if (option === "movie" || !option) {
            $.ajax({url: `https://api.themoviedb.org/3/search/movie?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US&page=1&query=${input}`, 
            }).done(function(data){
        ...
        if (option === "years") {
            ...
            $.ajax({url: `https://api.themoviedb.org/3/search/movie?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US&query=${movie}&year=${year}`, 
            }).done(function(data){
                const movies = data.results
                movies.forEach(movie => {
        ...
        if (option === "person") {
            $.ajax({url: `https://api.themoviedb.org/3/search/person?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US&query=${input}`
            }).done(function(data){
    ...
    $('.flex-box').on("click", ".element" ,function(){
        if( $(".element").attr('id') === "movie") {
            ...
            $.ajax({
                url: `https://api.themoviedb.org/3/movie/${movieFound.id}?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US`,
            }).done(function(data) {
                const movie = {
                    title: data.title,
            ...
            $.ajax({
                url: `https://api.themoviedb.org/3/movie/${movieFound.id}/credits?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US`
            }).done(function(data) {
    ...
    if( $(".element").attr('id') === "person") {
        ...
        $.ajax({
            url: `https://api.themoviedb.org/3/person/${personFound.id}?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US`
        }).done(function(data) {    
            const person = {
                name: data.name,
        ...
        $.ajax({
            url: `https://api.themoviedb.org/3/person/${personFound.id}/movie_credits?api_key=13fedccf58a8c7c6a64dcf50cb406fa5&language=en-US`
        }).done(function(data) {
            const crew = data.crew
            const cast = data.cast
